...
export function registerApplicants(bot) {
  bot.command('applicants', async (ctx) => {
    ...
    if (!id) return ctx.reply('Usage: /applicants <task_id>');
    ...
    await ctx.reply(
      [`Applicants for task #${id} (${assignedCount}/${task.maxAssignees} assigned):`, ...lines, '', `Use /assign <application_id> or /decline <application_id> [note].`].join('\n')
    );
  });
}
