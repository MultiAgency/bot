...
export function registerRoomAdmin(bot) {
  bot.command('addroomadmin', async (ctx) => {
    if (ctx.chat.type === 'private') return ctx.reply('ℹ️ Run this inside the group, not in a DM.');

    const replyTo = ctx.message.reply_to_message;
    if (!replyTo) {
      return ctx.reply('↩️ Reply to a message from the user you want to promote, then run /addroomadmin.');
    }
  ...
  bot.command('removeroomadmin', async (ctx) => {
    ...
    const replyTo = ctx.message.reply_to_message;
    if (!replyTo) {
      return ctx.reply('↩️ Reply to a room admin\'s message, then run /removeroomadmin.');
    }
