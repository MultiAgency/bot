...
export function registerApprove(bot) {
  bot.command('approve', async (ctx) => {
    ...
    if (result.count === 0) {
      ...
      return ctx.reply(`⚠️ Task #${id} is already ${current.status} - someone else may have just handled it.`);
    }
    ...
    await ctx.reply(
      `✅ Task #${id} approved and 🔓 open (max ${task.maxAssignees} assignee${task.maxAssignees === 1 ? '' : 's'}).\n📣 Notifying top-matched contributors to apply.`
    );
